...
static const char *insults[] = {
    "Still thinking? My grandma finishes bingo faster, and she's been dead for "
    "six years.",
    "At your current pace, the heat death of the universe will arrive before "
    "your second pick.",
    "I've seen faster decision-making from a Magic 8-Ball submerged in "
    "concrete.",
    "A golden retriever randomly slapping the keyboard would be quicker than "
    "you.",
    ...
    "The words aren't going to pick themselves. Or maybe they should, since "
    "you clearly can't.",
    "Even a stopped clock makes a decision twice a day. You: zero times so "
    "far.",
    "Scientists have confirmed that watching paint dry is more thrilling than "
    "watching you play.",
...
static const char *child_insults[] = {
    ...
    "I've been running for so long I've written a memoir. It's mostly about "
    "your slowness.",
...
#define NUM_CHILD_INSULTS                                                      \
  (int)(sizeof(child_insults) / sizeof(child_insults[0]))
...
static void child_main(int index) {
  ...
  while (1) {
    ...
    if (write(insult_pipe[1], msg, CHILD_MSG_LEN) < 0)
      break;
  }
  exit(0);
}
...
static void tui_move(int row, int col) { printf("\033[%d;%dH", row, col); }

/* Full TUI clear: erase screen, hide cursor, home */
static void tui_clear(void) { printf("\033[2J\033[H\033[?25l"); }
...
static void print_hidden_grid(void) {
  ...
  /* Total lines used: 1 title + 1 blank + GRID_DIM*(inner_h+1) + 1 last rule +
   * 2 footer */
...
static void print_final_result(char picked[][MAX_WORD_LEN]) {
  ...
  if (rl < 0)
    rl = 0;
  printf("%*s%*s" ANSI_BOLD ANSI_CYAN "%s" ANSI_RESET "%*s", left_off, "", ll,
         "", lhdr, gw - ll - (int)strlen(lhdr), "");
  printf("%*s", gap, "");
  printf("%*s" ANSI_BOLD ANSI_CYAN "%s" ANSI_RESET "\n\n", rl, "", rhdr);
...
static void print_selection_table(int cur_col, int cur_row,
  ...
  for (int c = col_start; c <= col_end; c++)
    if (col_heights[c] > max_rows)
      max_rows = col_heights[c];
  ...
  if (use_pages) {
    ...
    if (hl < 0)
      hl = 0;
    printf("%*s" ANSI_BOLD "Select one word per column  " ANSI_CYAN
           "(wasd/arrows | ENTER=confirm)  " ANSI_YELLOW
           "[Page %d/3: cols %d-%d]" ANSI_RESET "\n\n",
    ...
    if (hl < 0)
      hl = 0;
    printf("%*s" ANSI_BOLD "Select one word per column  " ANSI_CYAN
           "(wasd / arrows  |  ENTER=confirm column)" ANSI_RESET "\n\n",
           hl, "");
  }
      ...
      done++;
  {
    char status[256];
    int n =
        snprintf(status, sizeof(status), "Confirmed: %d / %d", done, SEL_COLS);
    int sl = (tw - n) / 2;
    if (sl < 0)
      sl = 0;
  ...
  if (insult_msg && insult_msg[0]) {
    ...
    const char *hint =
        use_pages ? "A/D: switch page  W/S: pick word  ENTER: confirm"
...
static int check_win(void) {
  ...
  if (selected[0] && selected[4] && selected[8])
    return 1;
  if (selected[2] && selected[4] && selected[6])
    return 1;
  return 0;
}
...
int main(void) {
  ...
  while (1) {
    ...
    /* Redraw */
    print_selection_table(cur_col, cur_row, col_confirmed, col_choice,
                          col_heights, all_words, total_words, current_insult);
    ...
    if (k == KEY_LEFT) {
      ...
      if (cur_row < col_heights[cur_col] - 1)
        cur_row++;
    } else if (k == KEY_ENTER) {
  ...
  int win = check_win();
  {
    ...
    tui_move(th, 1);
    const char *msg =
        win ? "*** BINGO!  You win!  A complete line cleared! ***"
